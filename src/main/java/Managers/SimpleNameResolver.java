...
@Service
public class SimpleNameResolver implements INameResolver {
    ...
    @Autowired
    public SimpleNameResolver(VocabularyManager vocabularyManager) {
    ...
    public String getDAOValueName(String typeDTO, String nameDTO) {
        String  result = nameDTO;
        if(this.vocabularyManager.isInVocabulary(typeDTO)){
            result = this.vocabularyManager.getIdByTypeAndValue(typeDTO, nameDTO);
        }
        return result;
    }
}
